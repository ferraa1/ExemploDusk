...
class EditTest extends DuskTestCase
{
    ...
    /**
     * Teste para verificar se o funcionário pode editar outros funcionários do sistema.
     *
     * @test
     */
    public function the_funcionario_can_edit()
    {
        ...
        $this->browse(function (Browser $browser) use ($funcionario) {
            $browser->visit('/')
                ...
                ->clickLink('Funcionários')
                ->clickLink('Cadastrar')
                ->type('nome', 'NOME_EDITADO_TESTE_DUSK')
                ->type('usuario', 'usuario_editado_teste_dusk')
                ->type('senha', '123')
                ->radio('admin', 1)
                ->check('concordar')
                ->press('acao')
                //confirmar
                ->type('filtro', 'NOME_EDITADO_TESTE_DUSK')
                ->press('Consultar')
                ->assertSee('NOME_EDITADO_TESTE_DUSK')
                ->assertSee('usuario_editado_teste_dusk');
            echo "\nFINALIZADO: EditTest\n";
        });
    }
}
